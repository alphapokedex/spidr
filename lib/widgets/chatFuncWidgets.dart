...
Widget callBtt({
  ...
  inCallUsers.removeWhere((key, value) => inCallUsers[key]["userId"] == Constants.myUserId);

  toChat()async{
    bool ready = await checkCamPermission(platform) && await checkMicPermission(platform);
    if(ready){
      Navigator.push(
        context,
        MaterialPageRoute(
  ...
  return inCallUsers.isEmpty ? IconButton(
    icon: Icon(platform == TargetPlatform.android ? Icons.phone : CupertinoIcons.phone_fill),
