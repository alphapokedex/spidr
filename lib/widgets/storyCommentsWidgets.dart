                        ...
                        child: TextFormField(
                            validator: (val) {
                              return !emptyStrChecker(val)
                                  ? val.length <= 300
                                      ? null
                                      : 'Sorry, comment > 300 characters'
                                  : 'Sorry, comment can not be empty';
                            },
                            ...
                            decoration: msgInputDec(
                                context: context, hintText: 'Comment')),
                      ),
                    ),
                    IconButton(
                      ...
                      color: Colors.orange,
                      onPressed: () async {
                        await addComment(comFormKey, commentEditingController,
                            storyId, storySenderId);
  ...
  return StreamBuilder(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasData && snapshot.data != null) {
          numOfCom = snapshot.data.docs.fold(
              0,
              (sum, e) => !Constants.myBlockList.contains(e.data()['senderId'])
                  ? sum + 1
                  : sum + 0);
        }
        return SizedBox(
