                        ...
                        ? StreamBuilder(
                            ...
                            builder: (context, snapshot) {
                              if (snapshot.hasData &&
                                ...
                                String hashTag =
                                    snapshot.data.data()['hashTag'];
                                String profileImg =
                                    snapshot.data.data()['profileImg'];
                                String groupState =
                                    snapshot.data.data()['chatRoomState'];
                                bool isMember = snapshot.data
                                    .data()['members']
                                    .contains(Constants.myUserId);
                                bool oneDay =
                                    snapshot.data.data()['oneDay'] != null &&
                                        snapshot.data.data()['oneDay'];
                                int createdAt =
                                    snapshot.data.data()['createdAt'];
                                int timeElapsed =
                                    oneDay ? getTimeElapsed(createdAt) : null;
                                      ...
                                      DatabaseMethods()
                                        ...
                                        if (val != -1) {
                                          ...
                                          Fluttertoast.showToast(
                                              msg:
                                                  'Sorry, this message has been deleted');
                                        }
                                      });
                                    } else {
                                      Fluttertoast.showToast(
                                          msg: 'Sorry, this circle is private');
                                    }
                                  },
                                  child: Container(
                                    decoration: const BoxDecoration(
                                    ...
                                    child: ListTile(
                                        ...
                                        subtitle: Text(
                                            groupState == 'public' || isMember
                                                ? 'view in circle'
                                                : 'private circle'),
                                        trailing: iconContainer(
                                          icon: groupState == 'private' &&
                                                  !isMember
                                              ? platform ==
  ...
  return await showModalBottomSheet(
      ...
      builder: (BuildContext context) {
        return DraggableScrollableSheet(
            ...
            minChildSize: 0.65,
            builder: (BuildContext context, ScrollController controller) {
              return UploadBttSheetWrapper(controller, groupId, personalChatId,
                  friend, contactId, numOfAvlUpl, uploadTo, singleFile);
            });
      });
}

openUserProfileBttSheet(
    BuildContext context, String userId, String username, String profileImg) {
  showModalBottomSheet(
      clipBehavior: Clip.antiAlias,
      shape: const RoundedRectangleBorder(
          borderRadius: BorderRadius.vertical(top: Radius.circular(30.0))),
