...
class MyCirclesScreen extends StatefulWidget {
  ...
  const MyCirclesScreen(this.mutedChats, {Key key}) : super(key: key);
  @override
...
class _MyCirclesScreenState extends State<MyCirclesScreen>
  ...
  acceptGroupInvite(String groupId, String hashTag, String groupState) async {
    DocumentSnapshot groupSnapshot = await DatabaseMethods().getGroupChatById(
      groupId,
    );
    ...
    if (numOfMem < groupCap) {
      DatabaseMethods(uid: Constants.myUserId).toggleGroupMembership(
        groupId,
        'JOIN_PUB_GROUP_CHAT',
      );
    } else {
  ...
  requestJoinPvtGroup(String groupId, String hashTag, String groupState) async {
    ...
    if (numOfMem < groupCap) {
      DatabaseMethods(uid: Constants.myUserId).requestJoinGroup(
        groupId,
        Constants.myName,
        Constants.myUserId,
        Constants.myEmail,
          ...
          children: [
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                child: Text(
                  groupState != 'private' ? 'Join' : 'Request',
                  style: const TextStyle(
                    fontWeight: FontWeight.bold,
                    color: Colors.white,
                  ),
                  ...
                  ? StreamBuilder(
                      ...
                      builder: (_, snapshot) {
                        if (snapshot.hasData && snapshot.data.data() != null) {
                          ...
                          return newBorderedText(hashTag, Colors.black);
                        }
                      },
                    )
                  : newBorderedText('Expired', Colors.grey),
            ],
          ),
        ],
      ),
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: TabBar(
                    ...
                    indicatorWeight: 4,
                    tabs: [
                      newSectionLabel('My Circles'),
                      newSectionLabel('Spectating Circles'),
                    ],
                  ),
                ),
                Expanded(
              ...
              int value = await showMenu(
                  ...
                  items: [
                    ...
                    PopupMenuItem(
                      value: 2,
                      child: Text(
                        !muted ? 'Mute Notification' : 'Unmute Notification',
                      ),
                    ),
                  ]);
              if (value == 1) {
                if (!pinned) {
                  DatabaseMethods(uid: Constants.myUserId).pinMyGroup(
                    groupId,
                  );
                } else {
                  DatabaseMethods(uid: Constants.myUserId).unPinMyGroup(
                    groupId,
                  );
                }
              } else if (value == 2) {
                if (!muted) {
                  DatabaseMethods(uid: Constants.myUserId).muteMyGroup(
                    groupId,
                  );
                } else {
                  DatabaseMethods(uid: Constants.myUserId).unMuteMyGroup(
                    groupId,
                  );
                }
              }
                      ...
                      children: [
                        ...
                        numOfUploads != null && numOfUploads > 0
                            ...
                            : numOfNewMsg != null && numOfNewMsg > 0
                                ...
                                : const SizedBox.shrink(),
                        storyStreamWrapper(
                          storyStream: DatabaseMethods(uid: Constants.myUserId)
