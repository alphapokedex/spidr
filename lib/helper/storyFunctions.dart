...
storyRemove(
  ...
  if (owner) {
    DatabaseMethods(uid: Constants.myUserId)
        .deleteSenderStory(storyId, mediaObj, mediaGallery);
  } else if (seen == null || seen) {
    DatabaseMethods(uid: Constants.myUserId)
        .deleteReceiverStory(storyId, friendId, groupId, type);
  }
}
...
addComment(formKey, TextEditingController commentEditingController,
    String storyId, String storySenderId) async {
  if (formKey.currentState.validate()) {
    String comment = commentEditingController.text;
    await DatabaseMethods(uid: Constants.myUserId)
        .addStoryComment(storyId, storySenderId, comment);
    commentEditingController.text = '';
  }
}

addReply(formKey, TextEditingController replyEditingController, String storyId,
    String commentId) async {
  if (formKey.currentState.validate()) {
    String reply = replyEditingController.text;
    await DatabaseMethods(uid: Constants.myUserId)
        .addCommentReply(storyId, commentId, reply);
